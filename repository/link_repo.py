...
class LinkRepository:
    ...
    def is_link_valid(self, link_id: int, group_auto_id: int) -> bool:
        stmt = select(Link).where(and_(Link.link_id == link_id, Link.group_auto_id == group_auto_id))
        if self.session.scalar(stmt) is None:
            return False
        return True

    def delete_link(self, link_id: int, group_id: int):
        stmt = delete(Link).where(and_(Link.link_id == link_id, Link.group_auto_id == group_id))
